...
class PDFParser:
        ...
        try: 
            ...
            for pageNumber in range(0, document.getNumberOfPages()):      
                ...
                if ((pageNumber < self.ocrMaxPageCount) or (self.ocrMaxPageCount == -1)) and ((self.GetSymbolsCount(parsedText) < self.ocrSymbolsPerPageThreshold) or (self.ocrSymbolsPerPageThreshold == -1)):
                    print('info','performing ocr on page {0} of pdf {1}'.format(pageNumber + 1, FileName))
                    ocrResp = self.PerformOCROnPage(document, pageNumber)

                    if not ocrResp.success:
                        print('info','could not perform ocr on page {0} of pdf {1} {2}'.format(pageNumber + 1, FileName, ocrResp.message))
                ...
                try:
                    ...
                    print('info','could not extract annotations from page {0} of pdf {1}'.format(pageNumber + 1, FileName))
    ...
    def GenerateThumbnail(self, document):
        try:           
            ...
            print('info','unable to generate thumbnail for pdf {0}'.format(str(ex)))
            return None 
